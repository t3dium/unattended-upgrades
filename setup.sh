...
schedule_cleanup(){
  ...
  if [ $cleanup_system == "y" ]; then
    #debian/ubuntu
    if [ $distro == "1" ]; then
      echo "@monthly apt auto-remove -y" >> crontab
    else
      #arch
      #below command pipes excplicit orphans (not optional ones) into pacman's full remove command.
      echo "@monthly pacman -Qtdq | pacman -Rns --noconfirm" | crontab
    fi
  fi
  ###########################################################################################################
  echo "$green Schedule monthly docker cleanup? Old images, unused volumes etc [Options] = [y] [n] $purple"
  read cleanup_docker
  if [ $cleanup_docker == "y" ]; then
    echo "@monthly docker system prune -f" | crontab
  fi
...
###########################################################################################################
echo "$green [Enter] [1] if you're running a debian/ubuntu based distro, or [2] if running arch $purple"
read distro
if [ $distro == "1" ]; then
...
###########################################################################################################
echo "$green Do you want to enable unattended upgrades for flatpaks? [Options] = [y] [n] $purple"
read flatpak
if [ $flatpak == "y" ]; then
...
###########################################################################################################
echo "$green Do you want to enable unattended upgrades for snaps? [Options] = [y] [n] $purple"
read snap
...
###########################################################################################################
echo "$green Do you want to enable unattended upgrades for python pip packages? [Options] = [y] [n] $purple"
read pip
if [ $pip == "y" ]; then
